...
- **Οθόνες αφής (`@media (pointer: coarse)`): τα ίδια εφέ, φθηνά.** Στο Chrome ένα animation με `filter: blur()` δεν τρέχει στον compositor — ξαναζωγραφίζει το στοιχείο στο main thread σε κάθε καρέ. Γι' αυτό στο κινητό: το θόλωμα των εισόδων λύνεται σε 3-6 βήματα (`steps()`) πάνω σε δικό τους layer (`will-change: transform`) ενώ ξεθώριασμα/ανάδυση μένουν ομαλά· η CSS ομίχλη κινείται αλλά είναι gradient χωρίς `filter`· μενού/cookie banner χωρίς `backdrop-filter` (σχεδόν αδιαφανές φόντο)· page-in μόνο στην πρώτη ενότητα· σκιές κειμένου με μικρό blur και η λάμψη του «PISMA.» ως gradient. Παντού: η λωρίδα φωτός του «PISMA.» είναι δύο transform animations (παράθυρο + αντίγραφο) αντί για `background-position` (ξαναζωγράφιζε 6 γράμματα σε κάθε καρέ), και όλα τα `left/top` animations έγιναν `transform`. Κανόνας για νέα εφέ: στο κινητό μόνο opacity/transform ανά καρέ — ποτέ blur/backdrop-filter/left/top/color/text-shadow/background-position σε κάθε καρέ.
- Μέτρηση «κολλήματος»: `scratchpad/lh/fps.mjs` (SWGL=1 = εξομοίωση αργής GPU) και `ablate.mjs` (απενεργοποίηση ενός εφέ τη φορά). Η παλιά έκδοση: 6-17 fps για 4 s μετά το φόρτωμα στο κινητό· τώρα 45-60.
